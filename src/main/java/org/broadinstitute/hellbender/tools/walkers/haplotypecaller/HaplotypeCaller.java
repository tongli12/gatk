 ...
 * recalibration (Best Practice) or hard-filtering before use in downstream analyses. If using the GVCF workflow, the output is a GVCF file that must first be run through GenotypeGVCFs and then
 ...
 *
 * <br />
 * <h4>Single-sample GVCF calling on DNAseq (outputs intermediate GVCF)</h4>
 * <pre>
 ...
 * </pre>
 *
 * <h4>Single-sample GVCF calling on DNAseq with <a href='https://software.broadinstitute.org/gatk/documentation/article?id=9622'>allele-specific annotations</a></h4>
 * <pre>
 * ./gatk-launch HaplotypeCaller  \
 *   -R reference.fasta \
 *   -I input.bam \
 ...
 * </pre>
 *
 * <h4>Variant-only calling on DNAseq with  <a href='https://software.broadinstitute.org/gatk/documentation/article?id=5484'>bamout</a> to show realigned reads</h4>
 * <pre>
 * ./gatk-launch HaplotypeCaller  \
 *   -R reference.fasta \
 *   -I input.bam \
 ...
 * RNAseq-specific functionalities. Use those in combination at your own risk.</li> To replace the multi-threading issues users reported in GATK3, we now have a Spark version of HaplotypeCaller.
 * </ul>
 *
 * <h3>Special note on ploidy</h3>
 * <p>This tool is able to handle almost any ploidy (we are working on very high ploidies in large pooled experiments); the ploidy can be specified using the -ploidy argument for non-diploid organisms.</p>
 *
